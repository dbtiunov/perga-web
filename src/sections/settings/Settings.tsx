...
const Settings = () => {
  ...
  return (
    <div className="container mx-auto px-4 py-6">
      {error && <div className="bg-red-100 border border-red-400 text-red-700 px-3 py-2 rounded mb-3 text-sm">
        {error}
      </div>}

      {successMessage && <div className="bg-green-100 border border-green-400 text-green-700 px-3 py-2 rounded mb-3 text-sm">
        {successMessage}
      </div>}

      <div className="px-6 pt-4 pb-6 mb-4 w-full md:max-w-1/3">
        <form onSubmit={handleSettingsUpdate} className="space-y-4">
          <div className="space-y-3 mb-6">
            ...
            <div className="flex space-x-4">
              <div className="flex items-center">
                <input id="sunday" type="radio" name="weekStartDay" value="sunday" checked={weekStartDay === 'sunday'}
                       onChange={() => setWeekStartDay('sunday')}
                       className="h-4 w-4 text-blue-600 focus:ring-blue-500" />
                <label htmlFor="sunday" className="ml-2 block text-sm text-gray-700">Sunday</label>
              </div>
              <div className="flex items-center">
                <input id="monday" type="radio" name="weekStartDay" value="monday" checked={weekStartDay === 'monday'}
          ...
          <div>
            <button type="submit" disabled={isUpdatingSettings || !hasChanges}
                    className={`${hasChanges ? 'bg-blue-500 hover:bg-blue-700' : 'bg-gray-400 cursor-not-allowed'} text-white font-medium py-1.5 px-8 rounded
